...
class MarketGroup(BasePage):
    ...
    def click_marketchannel(self):
        ...
        :return:
        """
        self.find_clickable_elem(self.click_marketchannel_loc).click()
    ...
    modify_click_mc_select_loc = (By.CSS_SELECTOR, configdata.get_elementinfo(20))

    def modify_click_mc_select(self):
        """
        编辑页面点击市场大区下拉框
        :return:
        """
        self.find_clickable_elem(self.modify_click_mc_select_loc).click()

    modify_sendkeys_mc_select_loc = (By.CSS_SELECTOR, configdata.get_elementinfo(21))

    def modify_sendkeys_mc_select(self, mc):
        """
        编辑页面市场大区下拉框输入市场大区精确搜索
        :return:
        """
        self.send_keys(self.modify_sendkeys_mc_select_loc, mc)

    modify_click_marketchannel_loc = (By.CSS_SELECTOR, configdata.get_elementinfo(22))

    def modify_click_marketchannel(self):
        """
        编辑页面点击选择精确搜索后的市场大区
        :return:
        """
        self.find_clickable_elem(self.modify_click_marketchannel_loc).click()
    ...
    select_mc_loc = (By.CSS_SELECTOR, configdata.get_checkelementinfo(9))
    select_delete_loc = (By.CSS_SELECTOR, configdata.get_elementinfo(29))
    def search_marketgroup(self, mc, number_mg, abbr_name_mg):
        """
        市场群查询:市场大区输入框有返回值时清除输入框（查询仅限于单市场大区查询）
        :return:
        """
        if self.find_presence_elem(self.select_mc_loc).text != "":
            self.find_clickable_elem(self.select_mc_loc).click()
    ...
    modify_select_mc_loc = (By.CSS_SELECTOR, configdata.get_checkelementinfo(10))
    ...
    def modify_marketgroup(self, modify_mc, modify_cn_name, modify_en_name, modify_mark):
        ...
        if self.find_presence_elem(self.modify_select_mc_loc).text != modify_mc:    # 区分大小写
            print('==============='+self.find_presence_elem(self.modify_select_mc_loc).text)
            self.modify_click_mc_select()
            self.modify_sendkeys_mc_select(modify_mc)
            sleep(1)
            self.modify_click_marketchannel()
        else:
            pass
...
if __name__ == '__main__':
    ...
    case2.add_marketgroup("CN", "test197", "测试11", "test11", "1111111111")
    sleep(3)
    case2.search_marketgroup('CN', '', 'test197')
    sleep(3)
    case2.modify_marketgroup('DTC', '测试11修改', 'test11modify', '111111111modify')
    sleep(3)
    case2.search_marketgroup('DTC', '', 'test197')