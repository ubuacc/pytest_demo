...
from public.models.sendemail import send_email
...
def add_case(test_path=setting.TEST_DIR):
    """加载所有的测试用例"""
    discover = unittest.defaultTestLoader.discover(test_path, pattern='*_process_case.py')
    return discover
...
def run_case(all_case,result_path=setting.TEST_REPORT):
    ...
    fp = open(filename,'wb')
    runner = HTMLTestRunner(stream=fp,title='AMDP-UI自动化测试报告',
